...
citiesHistory = JSON.parse(localStorage.getItem("weather"))
if (!citiesHistory) {
  citiesHistory = []
}
else{
  searchWeather(citiesHistory[citiesHistory.length-1][0],citiesHistory[citiesHistory.length-1][1])
}
rendersidebar()
...
function rendersidebar() {
  ...
  for (i = 0; i < citiesHistory.length; i++) {
    var list = `<li class="citySearch" index=${i}>${citiesHistory[i][0]}, ${citiesHistory[i][1]}</li>`
    $("#cityHistory").append(list);
...
searchWeather(citiesHistory[index][0], citiesHistory[index][1])
...
$("#submit").on("click", function (event) {
  ...
  if (inputCity === '' || inputState === '') {
    ...
    searchWeather(inputCity, inputState)

  }
})

function searchWeather(inputCity, inputState) {
  ...
  $.ajax({
    ...
    ////  localstorage   history =[[oskland,ca],[sf,ca],]
    var verify = citiesHistory.filter(elem => elem[0] === inputCity && elem[1]=== inputState)
    console.log("verify:", verify)
if (verify.length === 0){
    citiesHistory.push([inputCity,inputState]);
    console.log(citiesHistory);
    localStorage.setItem("weather", JSON.stringify(citiesHistory));
    rendersidebar()
}
